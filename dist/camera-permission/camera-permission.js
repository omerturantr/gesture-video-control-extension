...
var MODEL_FILENAME = "gesture_recognizer.task";
var FACE_MODEL_FILENAME = "face_landmarker.task";
var MEDIAPIPE_WASM_DIR = "vendor/mediapipe/wasm";
...
// src/gesture/classifier.ts
var GestureClassifier = class {
  analyze(result) {
  ...
  classifyHand(landmarks, handedness, cannedGesture) {
    ...
    const palmWidth = Math.max(this.distance(landmarks[5], landmarks[17]), 1e-3);
    const palmFacing = this.isPalmFacingCamera(landmarks, handedness);
    const orientation = palmFacing === null ? "unknown" : palmFacing ? "palm" : "back";
    ...
    const averageFingerTipY = [8, 12, 16, 20].reduce((sum, index) => sum + landmarks[index].y, 0) / 4;
    const averageFingerTipX = [8, 12, 16, 20].reduce((sum, index) => sum + this.mirrorX(landmarks[index].x), 0) / 4;
    ...
    const thumbExtended = thumbLength > palmWidth * 0.7 && this.distance(landmarks[4], palmCenter) > this.distance(landmarks[3], palmCenter) * 1.06;
    const thumbClearlyExtended = thumbExtended && thumbTipToPalm > palmWidth * 1.02 && thumbTipToIndexBase > palmWidth * 0.72;
    const thumbLikelyExtended = thumbExtended && thumbTipToPalm > palmWidth * 0.72 && thumbTipToIndexBase > palmWidth * 0.4;
    const thumbSidewaysExtended = thumbLength > palmWidth * 0.54 && thumbTipToIndexBase > palmWidth * 0.18 && thumbHorizontalSeparation > palmWidth * 0.1;
    const thumbClearlyTucked = thumbTipToPalm < palmWidth * 0.95 && thumbTipToIndexBase < palmWidth * 0.78;
    const fingerSpread = [8, 12, 16, 20].map((index) => this.distance(landmarks[index], palmCenter)).reduce((sum, value) => sum + value, 0) / 4;
    const averageFingerTipDistance = [8, 12, 16, 20].map((index) => this.distance(landmarks[index], palmCenter)).reduce((sum, value) => sum + value, 0) / 4;
    const fingerTipSpan = Math.max(...[8, 12, 16, 20].map((index) => this.mirrorX(landmarks[index].x))) - Math.min(...[8, 12, 16, 20].map((index) => this.mirrorX(landmarks[index].x)));
    ...
    const cannedOpenPalmMatch = cannedGesture.label === "OPEN_PALM" && cannedGesture.score >= 0.1;
    const relaxedOpenPalmGeometry = extendedCount >= 3 && curledCount <= 1 && thumbLikelyExtended && fingerSpread > palmWidth * 1.24 && averageFingerTipDistance > palmWidth * 1.3 && fingerTipSpan > palmWidth * 0.2 && orientation === "palm";
    const openPalmGeometry = extendedCount === 4 && curledCount <= 1 && thumbLikelyExtended && fingerSpread > palmWidth * 1.28 && averageFingerTipDistance > palmWidth * 1.34 && fingerTipSpan > palmWidth * 0.22 && orientation === "palm";
    const strongOpenPalmGeometry = openPalmGeometry && thumbClearlyExtended && fingerSpread > palmWidth * 1.36 && averageFingerTipDistance > palmWidth * 1.4;
    const debugBase = {
    ...
    if (strongOpenPalmGeometry || openPalmGeometry && cannedOpenPalmMatch || relaxedOpenPalmGeometry && cannedGesture.score >= 0.22) {
      const confidence = clamp(
        0.54 + (strongOpenPalmGeometry ? 0.1 : 0) + (thumbClearlyExtended ? 0.06 : 0) + (fingerSpread / palmWidth - 1.24) * 0.26 + (averageFingerTipDistance / palmWidth - 1.3) * 0.2 + cannedGesture.score * 0.28,
        0,
        0.98
      );
      return {
    ...
    const cannedClosedFistMatch = cannedGesture.label === "CLOSED_FIST" && cannedGesture.score >= 0.45;
    const cannedThumbHint = (cannedGesture.label === "THUMB_UP" || cannedGesture.label === "THUMB_DOWN") && cannedGesture.score >= 0.3;
    const thumbProtruding = thumbClearlyExtended || thumbHorizontalSeparation > palmWidth * 0.32 || thumbTipToPalm > palmWidth * 1.05 || thumbTipToIndexBase > palmWidth * 0.55;
    const fistThumbVeto = cannedThumbHint || thumbProtruding;
    const strongFistGeometry = !fistThumbVeto && curledCount >= 4 && extendedCount === 0 && thumbClearlyTucked && thumbHorizontalSeparation < palmWidth * 0.32 && fingerSpread < palmWidth * 1.18;
    const relaxedFistGeometry = !fistThumbVeto && curledCount >= 3 && extendedCount <= 1 && thumbClearlyTucked && thumbHorizontalSeparation < palmWidth * 0.3 && fingerSpread < palmWidth * 1.24;
    const cannedFistFallback = cannedClosedFistMatch && !cannedThumbHint && curledCount >= 3 && fingerSpread < palmWidth * 1.3;
    ...
    const fistShapeVeto = curledCount === 4 && fingerSpread < palmWidth * 1.1 && thumbTipToPalm < palmWidth * 1;
    if (!fistShapeVeto && curledCount >= 1 && extendedCount <= 2 && (thumbLikelyExtended || thumbSidewaysExtended)) {
      const absX = Math.abs(thumbVector.x);
      const absY = Math.abs(thumbVector.y);
      const directionStrength = Math.max(absX, absY) / Math.max(palmWidth, 1e-3);
      const thumbSeparation = Math.abs(thumbTipY - averageFingerTipY);
      const handLooksClosed = curledCount >= 2 && extendedCount <= 1 || curledCount >= 1 && extendedCount <= 2 && fingerSpread < palmWidth * 1.24 && averageFingerTipDistance < palmWidth * 1.3;
      const compactFingerCluster = fingerSpread < palmWidth * 1.34 && averageFingerTipDistance < palmWidth * 1.42 && fingerTipSpan < palmWidth * 0.34;
      const relaxedClosedLikeShape = curledCount >= 1 && fingerSpread < palmWidth * 1.4 && averageFingerTipDistance < palmWidth * 1.48 && fingerTipSpan < palmWidth * 0.42;
      const backFacingSeekShape = orientation === "back" && fingerSpread < palmWidth * 1.58 && averageFingerTipDistance < palmWidth * 1.62 && fingerTipSpan < palmWidth * 0.52;
      const thumbBeyondFingerCluster = thumbVector.x > 0 ? thumbTipX > maxFingerTipX + palmWidth * 0.02 : thumbTipX < minFingerTipX - palmWidth * 0.02;
      const thumbDirectionAligned = thumbVector.x > 0 ? thumbTipX > averageFingerTipX + palmWidth * 0.01 : thumbTipX < averageFingerTipX - palmWidth * 0.01;
      const thumbBaseDirectionAligned = thumbVector.x > 0 ? thumbTipX > thumbBaseX + palmWidth * 0.06 : thumbTipX < thumbBaseX - palmWidth * 0.06;
      const horizontalThumbHandShape = (handLooksClosed || compactFingerCluster || relaxedClosedLikeShape) && thumbTipToIndexBase > palmWidth * 0.1 && thumbLength > palmWidth * 0.58 && thumbBaseDirectionAligned;
      const cannedVerticalThumbMatch = (cannedGesture.label === "THUMB_UP" || cannedGesture.label === "THUMB_DOWN") && cannedGesture.score >= 0.35;
      const cannedVerticalHint = (cannedGesture.label === "THUMB_UP" || cannedGesture.label === "THUMB_DOWN") && cannedGesture.score >= 0.2;
      const geometricThumbMatch = thumbClearlyExtended && absY > absX * 1.4 && thumbSeparation > palmWidth * 0.4 && thumbSeparation > thumbHorizontalSeparation * 1.2;
      const cannedThumbMatch = cannedVerticalThumbMatch && thumbClearlyExtended && thumbSeparation > palmWidth * 0.18 && thumbSeparation > thumbHorizontalSeparation * 0.9;
      if (geometricThumbMatch || cannedThumbMatch) {
        const resolvedGesture = cannedVerticalThumbMatch && cannedGesture.label ? cannedGesture.label : thumbVector.y < 0 ? "THUMB_UP" : "THUMB_DOWN";
        const confidence = clamp(
          Math.max(
      ...
      const horizontalThumbMatch = !cannedVerticalHint && horizontalThumbHandShape && thumbDirectionAligned && absX > absY * 1.2 && thumbHorizontalSeparation > palmWidth * 0.16 && thumbHorizontalSeparation > thumbSeparation * 1.1 && thumbTipToPalm > palmWidth * 0.34;
      const backFacingHorizontalThumbMatch = !cannedVerticalHint && orientation !== "palm" && (compactFingerCluster || relaxedClosedLikeShape || backFacingSeekShape) && thumbBeyondFingerCluster && thumbBaseDirectionAligned && absX > absY * 1 && thumbHorizontalSeparation > palmWidth * 0.22 && thumbTipToPalm > palmWidth * 0.5;
      const strongBackFacingHorizontalThumbMatch = !cannedVerticalHint && backFacingSeekShape && thumbBeyondFingerCluster && thumbBaseDirectionAligned && absX > absY * 0.9 && thumbHorizontalSeparation > palmWidth * 0.2 && thumbTipToPalm > palmWidth * 0.42 && thumbLength > palmWidth * 0.5;
      if (horizontalThumbMatch || backFacingHorizontalThumbMatch || strongBackFacingHorizontalThumbMatch) {
        const gesture = thumbVector.x > 0 ? "THUMB_RIGHT" : "THUMB_LEFT";
        const confidence = clamp(
          0.58 + Math.max(curledCount - 1, 0) * 0.04 + directionStrength * 0.16 + (thumbHorizontalSeparation / palmWidth - 0.03) * 0.18 + (backFacingHorizontalThumbMatch ? 0.05 : 0) + (strongBackFacingHorizontalThumbMatch ? 0.04 : 0),
          0,
          0.96
        );
        return {
  ...
  isFingerExtended(landmarks, tipIndex, pipIndex, mcpIndex, palmCenter, palmWidth) {
  ...
  isFingerCurled(landmarks, tipIndex, pipIndex, palmCenter) {
  ...
  normalizeHandedness(handedness) {
    if (handedness === "Left") {
      return "Right";
    }
    if (handedness === "Right") {
...
var FaceAnalyzer = class {
  analyze(result) {
    ...
    const faceWidth = Math.max(this.distance(leftCheek, rightCheek), 1e-3);
    ...
    const verticalRatio = (nose.y - eyeMidY) / Math.max(mouthY - eyeMidY, 1e-3);
    const attentionState = horizontalOffset < 0.18 && verticalRatio > 0.18 && verticalRatio < 0.82 ? "looking" : "away";
  ...
  eyeAspectRatio(outer, inner, topA, bottomA, topB, bottomB) {
    const verticalA = this.distance(topA, bottomA);
    const verticalB = this.distance(topB, bottomB);
    const horizontal = Math.max(this.distance(outer, inner), 1e-3);
...
// src/gesture/session.ts
var GestureSession = class _GestureSession {
  static COMMAND_GESTURE_MIN_CONFIDENCE = 0.36;
  static OPEN_PALM_MIN_CONFIDENCE = 0.5;
  static OPEN_PALM_CONFIRMATION_FRAMES = 2;
  static CLOSED_FIST_CONFIRMATION_FRAMES = 2;
  static SEEK_HOLD_CONFIRMATION_FRAMES = 3;
  static VOLUME_HOLD_CONFIRMATION_FRAMES = 3;
  static STATIC_GESTURE_SWITCH_CONFIRMATION_FRAMES = 2;
  static HOLD_ACTION_SWITCH_CONFIRMATION_FRAMES = 3;
  static OPEN_PALM_RELEASE_FRAMES = 12;
  static CLOSED_FIST_RELEASE_FRAMES = 4;
  static SEEK_HOLD_RELEASE_FRAMES = 8;
  static THUMB_HOLD_RELEASE_FRAMES = 3;
  static STATIC_GESTURE_PERSISTENCE_MS = 320;
  static SEEK_GESTURE_PERSISTENCE_MS = 240;
  static FIST_SHORT_CIRCUIT_MIN_CONFIDENCE = 0.5;
  static HAND_PERSISTENCE_MS = 280;
  static HOLD_COMMAND_FAILURE_LIMIT = 3;
  static FACE_ANALYSIS_INTERVAL_MS = 90;
  static FACE_RESUME_TIMEOUT_MS = 5e3;
  static NO_SIGNAL_TIMEOUT_MS = 2500;
  ...
  frameTimerId = null;
  latestVideoTime = -1;
  oneShotCandidate = "NONE";
  oneShotFrames = 0;
  oneShotReleaseFrames = 0;
  latchedOneShot = "NONE";
  lastOneShotAt = /* @__PURE__ */ new Map();
  holdCandidateAction = "NONE";
  holdCandidateFrames = 0;
  activeHoldAction = "NONE";
  blockedHoldAction = "NONE";
  holdCommandFailureCounts = /* @__PURE__ */ new Map();
  activeHoldStartedAt = 0;
  activeHoldNextTriggerAt = 0;
  holdMissingFrames = 0;
  noSignalStartedAt = null;
  attentionStartedAt = null;
  eyeClosedStartedAt = null;
  faceResumeStartedAt = null;
  guardPauseLatched = false;
  guardPauseResumePending = false;
  faceResumeLatched = false;
  ...
  lastFaceAnalysisAt = -1;
  lastHandAnalysisAt = -1;
  lastStableStaticHandAt = -1;
  lastStableSeekHandAt = -1;
  constructor(options) {
  ...
  async start() {
    ...
    const visionFiles = await na.forVisionTasks(
      chrome.runtime.getURL(MEDIAPIPE_WASM_DIR)
    );
    this.recognizer = await Oc.createFromOptions(visionFiles, {
      baseOptions: {
    ...
    this.faceLandmarker = await Ic.createFromOptions(visionFiles, {
      baseOptions: {
  ...
  handleCommandFeedback(command, applied) {
    ...
    if (nextFailureCount >= _GestureSession.HOLD_COMMAND_FAILURE_LIMIT) {
  ...
  capturePreviewFrame(maxWidth = 220, quality = 0.72) {
    if (!this.isRunning || !this.stream || this.videoElement.videoWidth === 0 || this.videoElement.videoHeight === 0) {
      return null;
    }
  ...
  processFrame = () => {
    ...
    if (this.videoElement.readyState < HTMLMediaElement.HAVE_CURRENT_DATA || this.videoElement.currentTime === this.latestVideoTime) {
    ...
    try {
      ...
      if (snapshot.stableEvent) {
        this.commandQueue = this.commandQueue.then(() => this.onStableGesture(snapshot.stableEvent)).catch((error) => {
  ...
  consumeFrame(handAnalysis, faceAnalysis, timestamp) {
    ...
    const fistShortCircuit = primaryHand?.gesture === "CLOSED_FIST" && primaryHand.confidence >= _GestureSession.FIST_SHORT_CIRCUIT_MIN_CONFIDENCE;
    const holdAction = fistShortCircuit ? "NONE" : this.detectHoldAction(handAnalysis.hands);
    const holdEvent = fistShortCircuit ? this.consumeHoldAction("NONE", timestamp) : this.consumeHoldAction(holdAction, timestamp);
    const oneShotEvent = holdEvent === null ? this.consumeOneShotGesture(oneShotGesture, timestamp) : null;
    if (handAnalysis.handDetected) {
      this.resetFaceGuardState();
    }
    const { event: faceEvent, attentionElapsedMs, eyeClosureElapsedMs } = !handAnalysis.handDetected && holdEvent === null && oneShotEvent === null ? this.consumeFaceGuards(handAnalysis.handDetected, effectiveFaceAnalysis, timestamp) : {
      event: null,
      attentionElapsedMs: 0,
      eyeClosureElapsedMs: 0
    };
    const stableEvent = holdEvent ?? oneShotEvent ?? faceEvent;
    const effectiveGesture = stableEvent?.label && stableEvent.label !== "TRACKING" ? stableEvent.label : this.latchedOneShot !== "NONE" && handAnalysis.handDetected && this.activeHoldAction === "NONE" ? this.latchedOneShot : handAnalysis.displayGesture;
    const debug = this.settings.debugClassifier ? primaryHand?.debug : void 0;
    return {
  ...
  detectOneShotGesture(hands) {
    const primaryHand = hands[0];
    if (!primaryHand || primaryHand.confidence < this.getCommandMinConfidence(primaryHand.gesture)) {
      return "NONE";
    }
  ...
  detectHoldAction(hands) {
    const thumbGesture = hands.find(
      (hand) => this.isHoldGestureAboveThreshold(hand.gesture, hand.confidence)
    );
    const nextAction = thumbGesture ? this.mapGestureToHoldAction(thumbGesture.gesture) : "NONE";
    if (nextAction === "NONE") {
  ...
  isHoldGestureAboveThreshold(gesture, confidence) {
  ...
  static isOppositeHoldAction(left, right) {
  ...
  consumeOneShotGesture(gesture, timestamp) {
    const preset = SENSITIVITY_PRESETS[this.settings.sensitivity];
    if (gesture === "NONE") {
      ...
      if (this.latchedOneShot !== "NONE") {
        ...
        if (this.oneShotReleaseFrames >= this.getOneShotReleaseFrames(this.latchedOneShot, preset.releaseFrames)) {
    ...
    if (this.latchedOneShot !== "NONE" && gesture !== this.latchedOneShot) {
      ...
      if (this.oneShotFrames < _GestureSession.STATIC_GESTURE_SWITCH_CONFIRMATION_FRAMES || !this.isOffCooldown(gesture, timestamp, preset.oneShotCooldownMs)) {
        return null;
      }
    ...
    if (this.oneShotFrames < this.getOneShotConfirmationFrames(gesture, preset.oneShotConfirmationFrames) || !this.isOffCooldown(gesture, timestamp, preset.oneShotCooldownMs)) {
      return null;
    }
    return this.emitOneShotGesture(gesture, timestamp);
  }
  emitOneShotGesture(gesture, timestamp) {
  ...
  consumeHoldAction(holdAction, timestamp) {
    const preset = SENSITIVITY_PRESETS[this.settings.sensitivity];
    if (this.activeHoldAction !== "NONE") {
      ...
      if (holdAction !== "NONE") {
        ...
        } else if (_GestureSession.isOppositeHoldAction(this.activeHoldAction, holdAction)) {
        ...
        if (this.holdCandidateFrames < Math.max(
          this.getHoldConfirmationFrames(holdAction, preset.holdConfirmationFrames),
          _GestureSession.HOLD_ACTION_SWITCH_CONFIRMATION_FRAMES
        )) {
          return null;
        }
      ...
      if (holdAction === "NONE") {
        ...
        if (this.holdMissingFrames >= this.getHoldReleaseFrames(this.activeHoldAction, preset.releaseFrames)) {
          this.resetHoldState();
        }
        return null;
      }
    ...
    if (this.holdCandidateAction === holdAction) {
      this.holdCandidateFrames += 1;
    } else if (_GestureSession.isOppositeHoldAction(this.holdCandidateAction, holdAction)) {
      this.holdCandidateAction = holdAction;
      this.holdCandidateFrames = 0;
      return null;
    } else {
    ...
    if (this.holdCandidateFrames < this.getHoldConfirmationFrames(holdAction, preset.holdConfirmationFrames)) {
      return null;
    }
    this.activeHoldAction = holdAction;
    this.activeHoldStartedAt = timestamp;
  ...
  consumeFaceGuards(handDetected, faceAnalysis, timestamp) {
    const noSignalCondition = !handDetected && !faceAnalysis.faceDetected;
    const attentionCondition = this.settings.attentionGuardEnabled && faceAnalysis.faceDetected && faceAnalysis.attentionState === "away";
    const eyesClosedCondition = this.settings.eyeClosureGuardEnabled && faceAnalysis.faceDetected && faceAnalysis.eyeState === "closed";
    const resumeCondition = this.guardPauseResumePending && faceAnalysis.faceDetected && faceAnalysis.attentionState === "looking";
    ...
    const faceResumeElapsedMs = this.faceResumeStartedAt !== null ? timestamp - this.faceResumeStartedAt : 0;
    if (!noSignalCondition && !attentionCondition && !eyesClosedCondition) {
      this.guardPauseLatched = false;
    }
    if (!this.guardPauseLatched) {
      if (noSignalCondition && noSignalElapsedMs >= _GestureSession.NO_SIGNAL_TIMEOUT_MS) {
        this.guardPauseLatched = true;
        this.guardPauseResumePending = true;
        return {
          event: this.createEvent("TRACKING", "PAUSE", timestamp, {
            reason: `${(_GestureSession.NO_SIGNAL_TIMEOUT_MS / 1e3).toFixed(1)} saniye boyunca el veya y\xFCz alg\u0131lanmad\u0131\u011F\u0131 i\xE7in duraklat\u0131ld\u0131.`
          }),
          attentionElapsedMs: noSignalElapsedMs,
          eyeClosureElapsedMs
        };
      }
      if (eyesClosedCondition && eyeClosureElapsedMs >= this.settings.eyeClosureTimeoutSeconds * 1e3) {
        this.guardPauseLatched = true;
        this.guardPauseResumePending = true;
        return {
          event: this.createEvent("TRACKING", "PAUSE", timestamp, {
            reason: `G\xF6zler ${this.settings.eyeClosureTimeoutSeconds.toFixed(1)} saniye kapal\u0131 kald\u0131\u011F\u0131 i\xE7in duraklat\u0131ld\u0131.`
          }),
          attentionElapsedMs,
          eyeClosureElapsedMs
        };
      }
      if (attentionCondition && attentionElapsedMs >= this.settings.attentionTimeoutSeconds * 1e3) {
        this.guardPauseLatched = true;
        this.guardPauseResumePending = true;
        return {
          event: this.createEvent("TRACKING", "PAUSE", timestamp, {
            reason: `${this.settings.attentionTimeoutSeconds.toFixed(1)} saniye boyunca ekrandan uzak bak\u0131ld\u0131\u011F\u0131 i\xE7in duraklat\u0131ld\u0131.`
          }),
          attentionElapsedMs,
          eyeClosureElapsedMs
        };
    ...
    if (!this.faceResumeLatched && resumeCondition && faceResumeElapsedMs >= _GestureSession.FACE_RESUME_TIMEOUT_MS) {
      this.faceResumeLatched = true;
      this.guardPauseResumePending = false;
      return {
        event: this.createEvent("TRACKING", "PLAY", timestamp, {
          reason: `${(_GestureSession.FACE_RESUME_TIMEOUT_MS / 1e3).toFixed(1)} saniye boyunca ekrana bak\u0131ld\u0131\u011F\u0131 i\xE7in oynat\u0131ld\u0131.`
        }),
        attentionElapsedMs,
        eyeClosureElapsedMs
      };
  ...
  createEvent(label, command, timestamp, commandOptions) {
    return {
      label,
      command,
      confidence: 1,
  ...
  computeHoldInterval(holdDurationMs) {
    const preset = SENSITIVITY_PRESETS[this.settings.sensitivity];
    return Math.max(
  ...
  computeAccelerationMultiplier(holdDurationMs) {
    return 1 + holdDurationMs / 1e3 * (this.settings.holdAccelerationPercent / 100);
  }
  computeSeekAmount(holdDurationMs) {
    return Number((this.settings.seekSeconds * this.computeAccelerationMultiplier(holdDurationMs)).toFixed(2));
  }
  computeVolumeDelta(holdDurationMs) {
    return Number((this.settings.volumeStepPercent / 100 * this.computeAccelerationMultiplier(holdDurationMs)).toFixed(3));
  }
  computePlaybackRateDelta(holdDurationMs) {
    return Number((this.settings.playbackRateStep * this.computeAccelerationMultiplier(holdDurationMs)).toFixed(2));
  }
  isOffCooldown(label, timestamp, cooldownMs) {
    return timestamp - (this.lastOneShotAt.get(label) ?? 0) >= cooldownMs;
  }
  getCommandMinConfidence(gesture) {
    if (gesture === "OPEN_PALM") {
      return _GestureSession.OPEN_PALM_MIN_CONFIDENCE;
    }
    return _GestureSession.COMMAND_GESTURE_MIN_CONFIDENCE;
  }
  getOneShotConfirmationFrames(gesture, defaultConfirmationFrames) {
    if (gesture === "OPEN_PALM") {
      return Math.max(defaultConfirmationFrames, _GestureSession.OPEN_PALM_CONFIRMATION_FRAMES);
    }
    if (gesture === "CLOSED_FIST") {
      return Math.max(defaultConfirmationFrames, _GestureSession.CLOSED_FIST_CONFIRMATION_FRAMES);
    }
    return defaultConfirmationFrames;
  }
  getOneShotReleaseFrames(gesture, defaultReleaseFrames) {
    if (gesture === "OPEN_PALM") {
      return Math.max(defaultReleaseFrames, _GestureSession.OPEN_PALM_RELEASE_FRAMES);
    }
    if (gesture === "CLOSED_FIST") {
      return Math.max(defaultReleaseFrames, _GestureSession.CLOSED_FIST_RELEASE_FRAMES);
    }
  ...
  getHandAnalysis(nextHandAnalysis, timestamp) {
    if (nextHandAnalysis.handDetected) {
      if (nextHandAnalysis.displayGesture === "OPEN_PALM" || nextHandAnalysis.displayGesture === "CLOSED_FIST") {
        this.lastStableStaticHand = nextHandAnalysis;
        this.lastStableStaticHandAt = timestamp;
      }
      if (nextHandAnalysis.displayGesture === "THUMB_LEFT" || nextHandAnalysis.displayGesture === "THUMB_RIGHT") {
        this.lastStableSeekHand = nextHandAnalysis;
        this.lastStableSeekHandAt = timestamp;
      } else if (nextHandAnalysis.displayGesture === "TRACKING" && this.lastStableSeekHand.handDetected && this.lastStableSeekHandAt >= 0 && timestamp - this.lastStableSeekHandAt < _GestureSession.SEEK_GESTURE_PERSISTENCE_MS && this.lastStableSeekHandAt >= this.lastStableStaticHandAt) {
        this.lastHandAnalysis = this.lastStableSeekHand;
        this.lastHandAnalysisAt = timestamp;
        return this.lastStableSeekHand;
      } else if (nextHandAnalysis.displayGesture === "TRACKING" && this.lastStableStaticHand.handDetected && this.lastStableStaticHandAt >= 0 && timestamp - this.lastStableStaticHandAt < _GestureSession.STATIC_GESTURE_PERSISTENCE_MS) {
    ...
    if (this.lastHandAnalysis.handDetected && this.lastHandAnalysisAt >= 0 && timestamp - this.lastHandAnalysisAt < _GestureSession.HAND_PERSISTENCE_MS) {
      return this.activeHoldAction !== "NONE" ? this.lastHandAnalysis : {
        handDetected: true,
        hands: [],
  ...
  getHoldConfirmationFrames(action, defaultConfirmationFrames) {
    if (action === "SEEK_FORWARD" || action === "SEEK_BACKWARD") {
      return Math.max(defaultConfirmationFrames, _GestureSession.SEEK_HOLD_CONFIRMATION_FRAMES);
    }
    if (action === "VOLUME_UP" || action === "VOLUME_DOWN") {
      return Math.max(defaultConfirmationFrames, _GestureSession.VOLUME_HOLD_CONFIRMATION_FRAMES);
    }
    return defaultConfirmationFrames;
  }
  getHoldReleaseFrames(action, defaultReleaseFrames) {
    if (action === "SEEK_FORWARD" || action === "SEEK_BACKWARD") {
      return Math.max(defaultReleaseFrames, _GestureSession.SEEK_HOLD_RELEASE_FRAMES);
    }
    if (action === "VOLUME_UP" || action === "VOLUME_DOWN") {
      return Math.max(defaultReleaseFrames, _GestureSession.THUMB_HOLD_RELEASE_FRAMES);
    }
    return defaultReleaseFrames;
  }
  mapCommandToHoldAction(command) {
  ...
  mapGestureToHoldAction(gesture) {
  ...
  getFaceAnalysis(handDetected, timestamp) {
    if (handDetected) {
      this.lastFaceAnalysis = {
        faceDetected: false,
        attentionState: "unknown",
    ...
    if (this.lastFaceAnalysisAt >= 0 && timestamp - this.lastFaceAnalysisAt < _GestureSession.FACE_ANALYSIS_INTERVAL_MS) {
      return this.lastFaceAnalysis;
    }
    const faceResult = this.faceLandmarker?.detectForVideo(this.videoElement, timestamp);
    this.lastFaceAnalysis = faceResult ? this.faceAnalyzer.analyze(faceResult) : {
      faceDetected: false,
      attentionState: "unknown",
      eyeState: "unknown"
    };
...
var actionButton = document.querySelector("#allow-camera");
var STATE_SYNC_GRANULARITY_MS = 250;
if (!statusElement || !actionButton) {
  throw new Error("Kamera kontrol sayfas\u0131 ba\u015Flat\u0131lamad\u0131.");
}
var statusLabel = statusElement;
var cameraButton = actionButton;
var gestureSession = null;
var runtimeState = {};
var pendingSyncId = null;
var lastSyncedPayload = "";
function setStatus(message) {
  statusLabel.textContent = message;
}
function setActionButton(label, disabled = false) {
...
function formatControllerError(error) {
  ...
  return error instanceof Error && error.message ? `Kamera kontrol\xFC ba\u015Far\u0131s\u0131z oldu: ${error.message}` : "Bilinmeyen kamera kontrol hatas\u0131.";
}
...
function updateFromSnapshot(snapshot) {
  ...
  setStatus(
    snapshot.handDetected ? "Kamera haz\u0131r. El alg\u0131land\u0131. Video sekmenize d\xF6n\xFCn ve bu sayfay\u0131 a\xE7\u0131k b\u0131rak\u0131n." : "Kamera haz\u0131r. Videoyu kontrol ederken bu sayfay\u0131 a\xE7\u0131k b\u0131rak\u0131n."
  );
}
async function dispatchGestureCommand(event) {
  if (!event.command) {
    return;
  }
  mergeRuntimeState({
    ...
    lastCommandMessage: event.commandOptions?.reason ?? "Hareket komutu video sekmesine g\xF6nderiliyor.",
    error: ""
  });
...
async function startSession(settings) {
  if (gestureSession) {
    ...
    setStatus("Kamera kontrol\xFC zaten \xE7al\u0131\u015F\u0131yor.");
    setActionButton("Kontrol\xFC durdur");
    return;
  }
  mergeRuntimeState({
    status: "starting",
  ...
  try {
    ...
    mergeRuntimeState({
      status: "active",
      cameraStatus: "ready",
      lastCommandMessage: "Kamera kontrol\xFC etkin.",
      error: ""
    });
    scheduleStateSync(true);
    setStatus("Kamera haz\u0131r. Video sekmenize d\xF6n\xFCn ve bu sayfay\u0131 a\xE7\u0131k b\u0131rak\u0131n.");
    setActionButton("Kontrol\xFC durdur");
  } catch (error) {
...
async function stopSession(message = "Hareket kontrol\xFC durduruldu.") {
  if (gestureSession) {
...
async function toggleSession() {
  if (gestureSession) {
    try {
      ...
      await stopSession(response?.message ?? "Hareket kontrol\xFC durduruldu. Bu sekmeyi kapatabilirsiniz.");
    } catch {
      await stopSession("Hareket kontrol\xFC durduruldu. Bu sekmeyi kapatabilirsiniz.");
    }
    return;
  }
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  if (message.type === MESSAGE_TYPES.offscreenStop) {
    void stopSession("Hareket kontrol\xFC durduruldu.").then(() => {
...
window.addEventListener("beforeunload", () => {
  void stopSession("Hareket kontrol\xFC durduruldu.");
});
cameraButton.addEventListener("click", () => {
  void toggleSession();
});
async function initialize() {
  setStatus("Kamera kontrol\xFC a\xE7\u0131l\u0131yor.");
  setActionButton("Ba\u015Flat\u0131l\u0131yor...", true);
  const settings = await getStoredSettings();
  await startSession(settings);
}
void initialize();
//# sourceMappingURL=camera-permission.js.map
