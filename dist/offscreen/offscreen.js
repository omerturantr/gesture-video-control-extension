...
async function dispatchGestureCommand(event) {
  ...
  mergeRuntimeState({
    ...
    lastCommandMessage: event.commandOptions?.reason ?? "Hareket komutu sayfaya g\xF6nderiliyor.",
    error: ""
  });
...
async function startSession(settings) {
  ...
  try {
    ...
    mergeRuntimeState({
      status: "active",
      cameraStatus: "ready",
      lastCommandMessage: "Hareket kontrol\xFC arka planda \xE7al\u0131\u015F\u0131yor.",
      error: ""
    });
    scheduleStateSync(true);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Hareket kontrol\xFC ba\u015Flat\u0131lamad\u0131.";
    ...
    mergeRuntimeState({
      ...
      attentionElapsedMs: 0,
      eyeClosureElapsedMs: 0,
      lastCommandMessage: isPermissionError ? "Kamera izni reddedildi." : message,
      error: message
    });
    scheduleStateSync(true);
    throw error;
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  if (message.type === MESSAGE_TYPES.offscreenStart) {
    void startSession(message.settings).then(() => {
      sendResponse({ ok: true });
    }).catch((error) => {
      const responseMessage = error instanceof Error ? error.message : "GestureFlow ba\u015Flat\u0131lamad\u0131.";
